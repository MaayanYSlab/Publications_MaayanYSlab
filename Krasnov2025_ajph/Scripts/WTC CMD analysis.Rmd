...
```{r}
data<-readRDS(paste(wd,'\\long_term_preapred_full_data_2024-11-22.rds'))
...
```{r}
# 1. Diabetes
data_diab<-readRDS(paste (wd,'\\data_diab_2024-03-13.rds'))
data_diab<-data_diab[!duplicated(data_diab), ]

# 2. HTN
data_htn<-readRDS(paste (wd,'\\data_htn_2024-03-13.rds'))
data_htn<-data_htn[!duplicated(data_htn), ]

# 3. CAD
data_cad<-readRDS(paste (wd,'\\data_cad_2024-03-13.rds'))
...
```{r}
gluc = readRDS(paste(wd,'/gluc_beta_lcmm3_byAge75_140.rds'))
sys = readRDS(paste(wd,'/blood_pressure_lcmm2_linear_below130.rds'))
```
...
```{r}
traj_age <-readRDS(paste(wd,"/gluc_beta_lcmm3_byAge75_140.rds"))
...
# SBP
```{r}
traj_age <-readRDS(paste(wd,"/blood_pressure_lcmm2_linear_below130.rds"))
traj_age=subset(traj_age,select=c("ExtractID","class","year"))
traj_age = traj_age %>% group_by(ExtractID) %>% slice(n())
