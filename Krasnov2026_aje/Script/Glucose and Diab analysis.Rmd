...
1. Diabetes database
```{r}
data = readRDS(paste0(wd, "/diabHTNcad_for_analysis.rds")) %>%
...
output_file_path <- "/Figure S1.tiff"
...
2. WTC exposures
```{r}
wtc_data<-readRDS(paste0(wd,'/long_term_preapred_full_data_2024-11-22.rds'))
...
3. Glucose database
```{r}
data = readRDS(paste0(wd,"/biomarkers_for_analysis4.rds"))
...
# Figure 1:
```{r}
tiff("Figure1.tiff", units="in", width=6, height=5, res=300)
...
```{r}
# Figure 1:
tiff("/Figure_gluc_sex.tiff", units="in", width=6, height=5, res=300)
