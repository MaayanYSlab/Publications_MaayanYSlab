...
# Descriptive statistics of population for EHR3
...
a = table1(~ age + BMI2 +male + race5  +   Ischemic_stroke + pct_poverty + pct_black+ pct_NoHS + MEDHH + smoking | outcome , data=year_followup3,overall=F,render.continuous=my.render.cont, render.categorical=my.render.cat)
a
```

# supp 1. population characteristic by EHR2
```{r eval=FALSE, include=FALSE}
...
a = table1(~ race5  + male + BMI2 +pct_poverty + pct_black+ pct_NoHS + MEDHH + age | index_group , data=EHR2,overall=F,render.continuous=my.render.cont, render.categorical=my.render.cat)
...
df_corr = subset(EHR2, select = (corr_mix))
...
CreateTableOne(vars=listvars,data=EHR2)
```
...
for (i in seq_along(mixture)) {
 ...
 form <- paste0(
  ...
  mod = bam(as.formula(form), data = EHR2, family = poisson)
...
```{r}
# NMF
mod = bam(Ischemic_stroke ~ s(X1_app)+s(X2_app)+s(X3_app)+s(X4_app)+s(X5_app)+ as.factor(year) + as.factor(male) + as.factor(smoking)+ as.factor(race5)  + s(age)+ pct_poverty + pct_black + pct_NoHS+ MEDHH+s(sno,bs='re'), data = EHR2, family = poisson)
...
# sources comperison models - two year gap model
```{r}
