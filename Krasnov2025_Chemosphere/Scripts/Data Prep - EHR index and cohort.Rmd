...
```{r}
enc = read.csv(paste (wd,'/ENCOUNTER.csv'))
...
```{r}
#Read file
enc2 = read.csv(paste (wd,'/ENCOUNTER_COUNT.csv'))
...
```{r}
diag = read.csv(paste(wd,'/DIAGNOSIS.csv'))
...
```{r}
meds = read.csv(paste(wd,'/medication.csv'))
...
# Demographocs
```{r}
demog = read.csv(paste(wd,'/PATIENT.csv'))
...
# having labs
```{r}
labs = read.csv(paste(wd,'/labs.csv'))
...
#Having BMI recorded
```{r}
vit = read.csv(paste(wd,'/flowsheets.csv'))
...
# death
```{r}
patient=fread(paste(wd,'/PATIENT.csv'))
...
```{r}
ses_kev = readRDS(paste(wd,'/ses.rds'))
...
```{r}
comp = readRDS(paste(wd,'/comp_exposures_msdw2024.rds'))
